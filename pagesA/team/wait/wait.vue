		...
		<block v-if="teamInfo">
			<view class="header">
				...
				<view class="right">
					<block v-if="teamInfo.status == 0">
						<view class="tit">开团成功啦!</view>
						<view class="subtit">只差<text class="uni-red">{{teamInfo.surplus}}</text>位小伙伴参团，快点邀请好友参团。</view>
					</block>
					<block v-else-if="teamInfo.status == 1">
						<view class="tit">拼团成功啦!</view>
						<view class="subtit">继续去开团吧。</view>
					</block>
					<block v-else>
						<view class="tit">拼团失败啦!</view>
						<view class="subtit">只差<text class="uni-red">{{teamInfo.surplus}}</text>位小伙伴参团，继续去开团吧。</view>
					</block>
				</view>
			</view>
			<view class="goods-list">
				<view class="goods-item" @click="detailClick(teamInfo.goods_id)">
					...
					<view class="goods-right">
						...
						<view class="plan-box"><view class="shop-price">{{teamInfo.team_num}}人团</view></view>
						<view class="plan-box">
			...
			<view class="time-content">
				<view class="time-header">
					...
					<block v-else-if="teamInfo.status == 1"><div class="title-hrbg-team success">拼团成功</div></block>
					<block v-else><div class="title-hrbg-team error">拼团失败</div></block>
					<view class="hr"></view>
				</view>
				<view class="time-warp">
					<view class="picture">
						...
						<view class="col-box" v-for="(item,index) in teamUser" :key="index">
							<view class="tag-box" v-if="index == 0">团长</view>
					...
					<view class="progress">
						<view class="progress-with-pivot" :style="{width:pivotText}">
							<text class="progress-pivot">{{pivotText}}</text>
						</view>
					</view>
				</view>
			</view>
			...
			<view class="btn-bar">
				<button class="btn btn-org" @click="teamMore">更多拼团</button>
				<button class="btn btn-red" @click="goTeam" v-if="teamInfo.status > 0">去开团</button>
				<block v-else>
				<!-- #ifdef MP-WEIXIN -->
				<button class="btn btn-red" :data-id="teamInfo.goods_id" open-type="share">邀请好友参团</button>
				<!-- #endif -->
				<!-- #ifdef APP-PLUS -->
				<button class="btn btn-red" :data-id="teamInfo.goods_id" @tap="appShare">邀请好友参团</button>
				<!-- #endif -->
				</block>
			</view>
		</block>
	...
	export default {
		...
		onShareAppMessage(res){
			if (res.from === 'button') {// 来自页面内分享按钮
				return {
				  title: '我已开团成功，有一起来拼团的朋友吗?',
				  path: '/pagesA/team/detail/detail?goods_id='+res.target.dataset.id+'&team_id='+this.team_id
				}
			}else{
				return {
				  title: '我已开团成功，有一起来拼团的朋友吗?',
				  path: '/pagesA/team/wait/wait?team_id='+this.team_id+'&user_id='+ this.user_id
				}
			}
		},
		onLoad(e){
...
.time-warp .picture .col-box .img-box{ width: 100%; border:1px solid #ccc; border-radius: 50%; overflow: hidden; font-size: 0;}
.time-warp .picture .col-box .img-box image{ width: 100%; }
...
.time-warp .progress .progress-pivot{ position: absolute; right: 0; width: 60upx; height: 40upx; background: #ec5151; border-radius: 20upx; color: #FFFFFF; padding: 0 10upx; font-size: 24upx; top: -18upx; text-align: center;}
