					...
					<view class="goods-item" v-for="(item,index) in teamOrderData" :key="index">
						<view class="item-bd" @click="goodsLink(item.goods_id)">
							...
							<view class="goods-right">
								...
								<view class="plan-box"><view class="shop-price">已有{{item.limit_num}}人参团</view></view>
								<view class="plan-box"><view class="price">{{item.team_price}}</view></view>
							</view>
						</view>
						<view class="item-fd">
							<view class="btn-bar-min">
								<view class="btn btn-bor-red" v-if="item.is_pay == 0 && item.status == 0" @click="onlinepay(item.order_sn)">立即支付</view>
								<view class="btn" @click="detailLink(item.order_id)">查看订单</view>
								<view class="btn btn-bor-red" v-if="item.is_pay == 1 || item.status == 2" @click="waitHandle(item.team_id,item.user_id)">拼团进度</view>
							</view>
						</view>
						<view class="icon" v-if="status == 1"><image src="../../../static/s-bg.png" class="img" mode="widthFix" /></view>
						<view class="icon" v-if="status == 2"><image src="../../../static/sb-bg.png" class="img" mode="widthFix" /></view>
					</view>  
				</block>
				<block v-else>
