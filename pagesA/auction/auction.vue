			...
			<block v-if="auctionIndexData && auctionIndexData.length > 0">
				<view class="goods-item" v-for="(item,index) in auctionIndexData" :key="index" @click="detailHref(item)">
					...
					<view class="goods-right">
						...
						<view class="plan-box">
							<view class="shop-price" v-if="item.no_top == 0">一口价:{{item.formated_end_price}}</view>
							<view class="shop-price" v-else>不封顶</view>
						</view>
						<view class="plan-box">
							<view class="price">起拍价:{{item.formated_start_price}}</view>
