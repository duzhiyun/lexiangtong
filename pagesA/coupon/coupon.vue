					...
					<view class="left">
						<view class="coupon-item-con">
							...
							<view class="coupon-image" v-if="item.cou_is_receive == 1">
								<image src="../../static/coupons-print.png" class="img"></image>
							</view>
						</view>
					</view>
					<view class="right">
						<view v-if="item.cou_is_receive == 1" @click="couponLink(item.cou_id)">立即<br>使用</view>
						<view v-else-if="item.enable_ling == 1">已抢完</view>
						<view v-else @click="drawHandle(item.cou_id)">立即<br>领取</view>
					</view>
				</view>
			</view>
		</block>
...
<script>
	...
	export default {
		...
		methods: {
			...
			drawHandle(id){
				uni.request({
					url:this.websiteUrl + '/api/v4/coupon/receive',
					method:'POST',
					data:{
						cou_id:id
					},
