<template>
	<view class="drp-rank">
		<view class="header">
			<view class="img"><image src="../../../static/vip/rank_bg.jpg" mode="widthFix"></image></view>
			<view class="tit">您的排名：第 {{drpRankData.rank}} 名</view>
		</view>
		<view class="rank-box">
			<view class="title">
				<view class="t1">排名</view>
				<view class="t2">用户ID</view>
				<view class="t3">通过分享赚取</view>
			</view>
			<view class="list" v-for="(item,index) in drpRankData.list" :key="index" @click="teamHref(item.user_id)">
				<view class="left">
					<view class="icon">
				...
				<view class="right">
					...
					<uni-icon type="forward" size="20" color="#cccccc"></uni-icon>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			teamHref(user_id){
				uni.navigateTo({
					url:'/pagesA/drp/teamDetail/teamDetail?user_id='+user_id
				})
			}
		}
	}
...
.drp-rank .rank-box .list .con .name{ font-size: 30upx;}
