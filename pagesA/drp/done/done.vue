<template>
	<view>
		<view class="cashier-desk">
			<!-- #ifdef APP-PLUS -->
			<view class="header-title">需要支付：<view class="price">{{ doneinfo.order_amount_formated }}</view></view>
	...
	export default {
		...
		methods: {
			...
			weixinPay(e){
				...
				uni.getStorage({
					...
					complete: (res) => {
						//联系小韩 购买分销商
						uni.request({
							url: this.websiteUrl + '/api/v4/drp/wxappchangepayment',
							method: 'POST',
							data: {
			...
			getOrderInfo(pay_code) {
				uni.request({
					url: this.websiteUrl + '/api/v4/drp/changepayment',
					method: 'POST',
					data: {
						platform: uni.getStorageSync('platform'),
						pay_code:pay_code,
		...
		onLoad(e) {
			this.membership_card_id = e.membership_card_id ? e.membership_card_id : '';
			this.doneInfo();
			//获取支付方式
			// #ifdef APP-PLUS
			uni.getProvider({
