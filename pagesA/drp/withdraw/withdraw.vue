		...
		<view class="drp-withdraw">
			<form @submit="formSubmit">
				<view class="uni-form-item uni-column">
					<view class="title">当前可转出：<block v-if="drpTransData.max_money">{{drpTransData.max_money}}交易积分</block></view>
					<view style="width:95%;margin:0 auto;display: flex;align-items:center; flex-direction:row"> 
					    <view class="drp-duzytitle">输入转出数量</view><view style="border-bottom:1px solid #000000;"><input class="uni-input" type="number" v-model="amount" focus placeholder="请输入转出交易积分" /></view>		
					</view>
					
					<view class="sub">交易积分<block v-if="money">{{money}}</block>,最多可转<block v-if="drpTransData.max_money">{{drpTransData.max_money}}</block></view>
					<view class="sub"></view>
				</view>
				<view class="btn-bar btn-bar-radius">
					<button class="btn btn-red" formType="submit">转出到余额</button>
				</view>
			</form>
		</view>
		<dsc-common-nav>
...
<script>
	...
	export default {
		...
		methods: {
			load(){
				this.$store.dispatch('setDrpTrans',{type:2});
			},
			formSubmit(e){
