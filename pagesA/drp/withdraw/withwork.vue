		...
		<view class="drp-withdraw">
			<form @submit="formSubmit">
				<view class="uni-form-item uni-column">
					<view class="title">当前可转出：<block v-if="drpTransData.max_money">{{drpTransData.max_money}}乐券</block></view>
					<view style="width:95%;margin:0 auto;display: flex;align-items:center; flex-direction:row"> 
					    <view class="drp-duzytitle">输入转出数量</view><view style="border-bottom:1px solid #000000;"><input class="uni-input" type="number" v-model="amount" focus placeholder="请输入转出乐券" /></view>		
					</view>
					
					<view class="sub">余额<block v-if="money">{{money}}</block>,最多可转<block v-if="drpTransData.max_money">{{drpTransData.max_money}}</block></view>
					<view class="sub">注：乐券转余额不扣乐享值</view>
				</view>
