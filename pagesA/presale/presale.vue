<template>
	<view class="presale">
		<block v-if="presaleData">
			<view class="presale-list" v-for="(item,index) in presaleData" :key="index" v-if="item.goods != ''">
				<view class="title">
					<view class="text">{{ item.cat_name }}</view>
					<view class="more" @click="$outerHref('/pagesA/presale/list/list?cat_id=' + item.cat_id,'app')">更多<uni-icon type="forward" size="18" color="#999999"></uni-icon></view>
				</view>
...
.presale-list .title .more{ display: flex; justify-content: flex-end; align-items: center; color: #999999;}
...
.already-over{ width: 100upx; height: 100upx; position: absolute; bottom: 10upx; right: 20upx; z-index: 99;}
</style>
