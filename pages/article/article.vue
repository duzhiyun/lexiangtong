	...
	<view class="article">
		<view class="uni-tab-bar">
			<scroll-view id="tab-bar" class="uni-swiper-tab" scroll-x :scroll-left="scrollLeft">
				<view class="swiper-tab-list" :class="cat_id == 0 ? 'active' : ''" @click="handleArticle(0)">全部分类</view>
			...
			<view class="article-box">
				<block v-if="articleList && articleList.length > 0">
					<view class="article-list" v-for="(item,index) in articleList" :key="index" @click="$outerHref('/pages/article/detail/detail?id='+item.id,'app')">
						<view class="title">{{item.title}}</view>
						<view class="foot">
							<view class="author">{{item.author}}</view>
							<view class="option">
								<view class="time"><icon class="iconfont icon-shijian"></icon>{{ item.amity_time }}</view>
								<view class="click"><icon class="iconfont icon-liulan"></icon>{{ item.click }}</view>
							</view>
						</view>
					</view>
