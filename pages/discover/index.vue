		...
		<view class="community">
			<view class="item" @click="onTabs(1)">
				...
				<view class="theme-right">
					<text class="tit">讨论帖</text>
					<text class="span" v-if="communityData.tao">{{ communityData.tao.num }}条</text>
				</view>
			</view>
			<view class="item" @click="onTabs(2)">
				<view class="theme-left tm-zs"><icon class="iconfont icon-wenda"></icon></view>
				<view class="theme-right">
					<text class="tit">问答帖</text>
					<text class="span" v-if="communityData.wen">{{ communityData.wen.num }}条</text>
				</view>
			</view>
			<view class="item" @click="onTabs(3)">
				<view class="theme-left tm-ls"><icon class="iconfont icon-quanzi"></icon></view>
				<view class="theme-right">
					<text class="tit">圈子贴</text>
					<text class="span" v-if="communityData.quan">{{ communityData.quan.num }}条</text>
				</view>
			</view>
			<view class="item" @click="onTabs(4)">
				<view class="theme-left tm-hs"><icon class="iconfont icon-paizhao"></icon></view>
				<view class="theme-right">
					<text class="tit">晒单贴</text>
					<text class="span" v-if="communityData.sun">{{ communityData.sun.num }}条</text>
				</view>
			</view>
		</view>
...
<script>
	...
	export default{ 
		...
		methods:{
			...
			onTabs(val){				
				uni.navigateTo({
					url:'/pages/discover/listType/listType?type='+val
				})
			},
			handleLikeNum(obj){
