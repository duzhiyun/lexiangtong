	...
	<view>
		<view class="mybuy border-b" v-for="(item,index) in list" :key="index">
			...
			<view class="list-ul">
			   ...
			   <view class="list-li txtcen titles">数量 ：{{item.number}}</view>
			</view>
			<view class="list-ol" v-if="item.status > 0">
			   <view> 手机号码 </view>
			   <view class="textblue" @click="call(item.user_name)">{{item.user_name}}</view>
			</view>
			<view class="list-ol" v-if="item.status > 0">
			   <view> 支付宝帐号 </view>
			   <view class="textblue" @click="copy(item.alipay)">{{item.alipay}}</view>
			</view>
			<view class="list-ol" v-if="item.status > 0">
			    <view>银行名称：{{item.bank}}</view>
				<view class="textblue" @click="copy(item.banknumber)">银行帐户：{{item.banknumber}}</view>			  
			</view>		
			
			<view class="list-ul bt30">
			   <view class="list-li hhimg" v-if="item.status == 0">
				   <view class="fonntsize10"></view>			   				   
				   <view><image :src="cancel" class="pzimg"></image></view>
				   <view class="hhak" @click="buyclick(item.id,5)">取消购买</view>
			   </view>
			   <view class="list-li hhimg" v-else-if="item.status == 1">
				   <view class="fonntsize10">请上传打款凭证</view>			   				   
				   <view @click="chooseImage('front_of_id_card',index)"><image :src="item.buy_image" class="pzimg"></image></view>
				   <view class="hhak" @click="buyclick(item.id,3)">上传凭证</view>
			   </view>
			   <view class="list-li hhimg" v-else-if="item.status == 2">
				   <view class="fonntsize10">已上传</view>
				   <view @click="chooseImage('front_of_id_card',index)"><image :src="item.buy_image" class="pzimg"></image></view>
			       <view class="hhak" @click="buyclick(item.id,3)">重新上传</view>
			   </view>
			   ...
			   <view class="list-li hhimg" > 
				   ...
				   <view class="hhok" v-if="item.status == 1" @click="buyclick(item.id,3)">上传凭证</view>
					...
					success: (res) => {
						if(res.data.status == 'success'){
							if(size){
								...
								let len = res.data.data.length;
								for(let i=0;i<len;i++){
									if(res.data.data[i].buy_image == ''){
										res.data.data[i].buy_image = '../../../static/hxsimg/errorImage.jpg'
									}
								}
						...
						pathToBase64(res.tempFilePaths[0]).then(base64 => {
							this.$store.dispatch('setMaterial',{
								...
								if(data.status == 'success'){
									this.list[i].buy_image = data.data[0]
