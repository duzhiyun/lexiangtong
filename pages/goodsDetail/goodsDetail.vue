<template>
	<view class="container">
		...
		<scroll-view class="scrollList" scroll-y :scroll-into-view="scrollViewId" :style="{height:winHeight+customBar + 'px'}">
			<view class="goods-wrap" id="tab0">
				...
				<view class="uni-card uni-card-not dlnone">
					<view class="uni-list">
			...
			<view class="goods-detail-guess dlnone" id="tab1" v-if="goodsGuessList" >
...
<script>
	...
	export default {
		...
		computed:{
			...
			gallerylList() {
				if(this.goodsInfo && this.goodsInfo.gallery_list){
			...
			attr() {
				if(this.$store.state.goods.goodsInfo && this.$store.state.goods.goodsInfo.attr){
...
<style>
	...
	.share{ display: flex; justify-content: center; align-items:flex-end; }
	.share .iconfont{ font-size: 14px;}
    .status_bar {
        height: var(--status-bar-height);
        width: 100%;
		background-color:#e61805s;
    }
