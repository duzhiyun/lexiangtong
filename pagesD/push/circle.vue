  ...
  <view id="moments"
      ...
      <view class="post_right">
        ...
        <view class="pay_view"
              v-if="post.can_pay == 1"> <button :disabled="post.can_pay_times == 0"
                  ...
                  size="mini"
				   @tap="open(index)"
                  type="primary">{{post.can_pay_times == 0 ? '已支付￥' : '￥'}} {{post.pay_amount}} 元</button></view>
    ...
    <view :class="['action_main animated faster',(action_menu ? 'bounceInDown' : 'bounceOutUp')]"
	  ...
	  <view v-show="showVedio"
			class="showVedio" style="position: fixed;" @tap="showVedio = false" >
		  <!-- <video :src="videoPath"
...
export default {
  ...
  onShow () {
    ...
    if (circle_data) {
      ...
      let userAddress =_data.localData('data_user_address');
    if(userAddress)_this.userAddress = userAddress;
  ...
  onNavigationBarButtonTap (e) {//监听标题栏点击事件
    if (e.index == 0) {
      ...
      uni.showActionSheet({
        ...
        success: res => {
          uni.navigateTo({
            url: './circle/send?type=' + list[res.tapIndex].key,
          });
        },
      });
    }
  ...
  methods: {
    ...
    goPublish () {
      ...
      open(index){
          ...
          if( this.my_data.id == data.uid){
               uni.showToast({title:'不能购买自己的商品哦!',icon:'none'});
               return false;
          };
