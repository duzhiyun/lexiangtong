								...
								<label class="uni-list-cell uni-list-cell-pd">
									...
									<view class="uni-media-list">
										<view class="uni-media-list-logo">
											<image  :data-index="index" :src="staticPhoto + item.photo" :lazy-load="true" style="border-radius: 50%;" @error="imageError"  />
										</view>
	...
	export default {
			...
			_get.getFriendList(send_data,(data) => {
				...
				for(let i in data_list ){
					for(let k = 0,o = data_list[i].data,l = o.length;k < l;k ++){
						o[k].other = data.member.indexOf(o[k].user_id) > -1 ? true : false;
					}
		...
		onNavigationBarButtonTap(e) {
			...
			if(_this.add_user_ids.length){
				let path = _this.list_id != '0' ? '/im/message/addChat' : '/im/message/addGroup';
				_this.$httpSend({
					path: path,
