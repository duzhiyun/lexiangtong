		...
		<view class="content uni-flex uni-flex-item">
			...
			<block v-if="bNews">
				<swiper class="swiper" autoplay="true" circular="true" vertical="true" interval="5000">
					<swiper-item class="swiper-item" v-for="(item, index) in announcementList" :key="index" @click="$outerHref(item.url)">
						<view class="tit uni-ellipsis">{{item.title}}</view>
						<view class="last" v-if="bDateSel">{{item.date}}</view>
					</swiper-item>
				</swiper>
				<view class="more-link">
					<view class="txt" @click="$outerHref('/pages/article/article','app')">更多</view>
...
export default{
	...
	created(){
		if(this.bNews){
			uni.request({
				url: this.websiteUrl + '/api/v4/visual/article',
				method: 'POST',
				data: {
					cat_id: this.catId || 0,
					num: this.nNumber
...
.announcement .icon{ width: 80upx; height: 80upx; vertical-align: middle; display: flex; align-items: center; margin: 10upx 20upx 10upx 0;}
.announcement .swiper{ flex: 1 1 0%; }
