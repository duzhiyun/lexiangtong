<template>
    <view class="jigsaw" :class="aJigsawClass">
		<view class="content">
			<view class="big-img" :class="aClass" :style="{'width':aStyle1Width0+'%'}" v-if="styleSel == '0' && module.list[0]" @click="$outerHref(module.list[0].appPage ? module.list[0].appPage : module.list[0].url,module.list[0].appPage ? 'app' : 'undefined')">
			...
			<view :class="aClass" :style="{'width':aStyle1Width1+'%'}">
				<view class="items" v-if="styleSel == '0'">
					<view class="item" v-for="(item, index) in imgList" :key="index" :class="s2Class" :style="{'width':style1RightW}" @click="$outerHref(
				...
				<view class="items" v-else>
					<view class="item" v-for="(item,index) in imgList" :key="index" :class="s2Class" :style="{'width':aStyle2Width[index]+'%'}" @click="$outerHref(item.appPage ? item.appPage : item.url,item.appPage ? 'app' : 'undefined')">
						<view class="desc" v-if="item.desc">{{ item.desc }}</view>
						<view class="icon"><image :src="item.img" mode="widthFix" v-if="item.img" class="image"></image></view>
					</view>
				</view>
