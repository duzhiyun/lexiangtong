			...
			<cell v-for="(list, idx) in lists" :key="idx" :ref="'uni-indexed-list-' + list.key">
				...
				<scroll-view :scroll-into-view="scrollViewId" class="uni-indexed-list__scroll" scroll-y>
					<view v-for="(list, idx) in lists" :key="idx" :id="'uni-indexed-list-' + list.key">
						<!-- #endif -->
						<uni-indexed-list-item :list="list" :loaded="loaded" :idx="idx" :showSelect="showSelect" :showBrand="showBrand" @itemClick="onClick"></uni-indexed-list-item>
						<!-- #ifndef APP-NVUE -->
					</view>
				</scroll-view>
				<!-- #endif -->
			...
			setList() {
				...
				if(this.showBrand){
					this.options.forEach((value, index) => {
						...
						let items = value.list.map(item => {
							let obj = {}
							// for (let key in item) {
							obj['key'] = item.info
							obj['name'] = item.brand_name
							obj['logo'] = item.brand_logo
