				...
				<block v-if="listMode == 'comlist'">
					<view @click="onDiscoverDetail(item.dis_type,item.dis_id)">
						<view class="com-min-tit">
							<uni-tag text="讨" size="small" type="error" v-if="item.dis_type == 1"></uni-tag>
							<uni-tag text="问" size="small" type="error" v-else-if="item.dis_type == 2"></uni-tag>
							<uni-tag text="圈" size="small" type="error" v-else-if="item.dis_type == 3"></uni-tag>
...
export default{
	...
	methods:{
		onDiscoverDetail(type,id){
			uni.navigateTo({
				url:'/pages/discover/detail/detail?dis_type='+ type + '&dis_id=' + id
			})
		},
		onZan(type,id){
