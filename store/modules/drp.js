...
const mutations = {
	...
	[DRP_CATEGORY_LIST](state,{o,res}){
		if(o.id){
			state.cateListSecond = res.data.data;
			state.drptype = res.data.type;
		}else{
...
const actions = {
   // 开店
	...
	setDrpCategoryLists({commit}, o){
		apiDrp.getDrpCategoryLists(o).then(res =>{
			commit(DRP_CATEGORY_LIST,{o,res})
		})
	},
	...
	setDrpWxappPay({commit},o){
		return apiDrp.getDrpWxappPay(o)
	},
    setDrpPay({ commit }, o){
	...
	setDrpApplyList({commit}){
		return apiDrp.getDrpApplyList()
	},
	setDrpApply({commit},o){
		return apiDrp.getDrpApply(o)
	},
	setDrpRenew({commit},o){
		return apiDrp.getDrpRenew(o)
	}
}
export default {
    state,
