		...
		<view class="shop">
			<view class="shop-top" v-for="(item,index) in shopCollectList" :key="index">
				<view class="left" @click="$outerHref('/pages/shop/shopHome/shopHome?ru_id='+item.ru_id,'app')">
					...
					<view class="info">
						<view class="name uin-ellipsis">{{ item.shoprz_brandName }}{{ item.shopNameSuffix }}</view>
						<view class="out">{{ item.count_store }}人关注</view>
					</view>
				</view>
				<view class="right">
					<text class="btn btn-red" @click="collectHandle(item.shop_id)">取消关注</text>
				</view>
			</view>
		</view>
		</block>
