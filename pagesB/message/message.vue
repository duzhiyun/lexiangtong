<template>
	<view class="fixed-bottom-padding">
		...
		<view class="btn-goods-action">
			<view class="submit-bar-text submit-bar-text-left"><input name="msg_title" v-model="msg_title" placeholder="说点什么吧～"/></view>
			<view class="btn-bar">
				<view class="btn btn-red" @click="onSubmit">提交</view>
			</view>
		</view>
	</view>
</template>
...
<script>
	export default {
		...
		methods:{
			default(){
				uni.request({
					url:this.websiteUrl + '/api/v4/feedback',
					method:'GET',
					data:{ 
						page:1,
						size:10
			...
			onSubmit(){
				if(this.msg_title != ''){
					var that = this;
					uni.request({
						url:this.websiteUrl + '/api/v4/feedback/create',
						method:'POST',
						data:{ 
					...
					uni.showToast({ title: '输入内容不能为空', icon: "none" });
				}
			}
		},
		onLoad(){
