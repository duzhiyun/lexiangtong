<template>
	<view>
		...
		<view class="fabu-top" :class="[tab=='a'?'taba':'']">
			...
			<view class="footer">
				<view @click="commit()" style="color:#ffffff;font-size:36rpx"> 确认修改</view>
			</view>
		</view>
				...
				uni.request({
					...
					success: (res) => {
						if(res.data.status == 'success'){
							...
							this.taskimage = this.item.tushi
							this.cindex = this.item.cid
							this.aindex = this.item.is_automatic
							this.lindex = this.item.linktype
							this.link = this.item.link
							this.end_date = this.item.end_date
							let olen = this.item.step_info.length
							for(let i=0;i<olen;i++){
								if(this.item.step_info[i] != ''){
									this.list.push(this.item.step_info[i])
									this.image.push(this.item.step_info[i])
									this.detail.push(this.item.description[i])
								}
							}
						}else{
