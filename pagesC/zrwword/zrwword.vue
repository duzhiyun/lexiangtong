	...
	<view>
		<view class="fabu-top">
			...
			<view class="fabu-nav" style="border-top:1px solid #515151;margin-top:40rpx;border-bottom:1px solid #515151;"> 
				<view class="fabu-imgteam"><text>{{item.apply_num}}人已赚 </text></view>
				<view class="fabu-imgteam"><text>剩余{{item.max_num-item.apply_num}}人</text></view>
				<view class="fabu-imgteam"><text>编号:{{id}}</text></view>
		...
		<view class="list-ul " v-if="item.tstatus!=2" @click="chooseImage('front_of_id_card')">
			<view class="list-name" style="flex-shrink:0" > 上传截图：</view>
			<image src="../../static/weike/upload.png" mode="widthFix" class="imgwidth"></image>	
		</view>
		<view class="list-ul" v-if="showa">
			<view class="list-name"> 任务截图：</view>
			<view class="list-inp">						
				 <view  class="inp-100 fy" style="text-align:left">
					 <image :src="aimage[0]"  mode="widthFix" class="imgwidth"></image>
					 <image :src="aimage[1]" v-if="showb" mode="widthFix" class="imgwidth"></image>
					 <image :src="aimage[2]" v-if="showc" mode="widthFix" class="imgwidth"></image>
				  </view>
			</view>					
		</view>
		...
		<view class="fabu-three">
			<view style="width:94%;margin:30rpx auto" v-for="(item,index) in description" :key="index">
				...
				<view>
					<image :src="image[index]" style="width:500rpx;margin:80rpx" mode="widthFix"></image>
				</view>
			</view>
			<view style="width:94%;margin:30rpx auto" v-for="(item,index) in oimage" :key="index">
				<view>
					<image :src="item" style="width:500rpx;margin:80rpx"  mode="widthFix"></image>
				</view>
			</view>
		</view>
		<view class="footer">			 
			<view   @click="commit()" style="color:#ffffff ;font-size:36rpx" v-if="item.tstatus==0">确认提交</view>
			<view   @click="commit()" style="color:#ffffff; font-size:36rpx" v-if="item.tstatus==1">确认修改</view>
			<view   style="color:#ffffff;font-size:36rpx" v-if="item.tstatus==2">任务已完成</view>
		</view>
...
<style>
.fy{display: block}
.inp-100{line-height:70upx;color:#000000;font-size:24rpx;min-width: 70rpx;}
.list-inp{width:70%;     
		line-height:70rpx;
		text-align:center;
		color:#000000;
...
.fabu-title{color:#ffffff;line-heighe:30rpx;margin-left:20rpx;font-size:36rpx;white-space:nowrap;overflow:hidden ; text-overflow:ellipsis;}
.fabu-nav{
	width:100%;
	line-height:30rpx;
	display: flex;
...
.fabu-team{font-size:20rpx;width:20%;margin:10rpx 2%;border-radius: 50rpx;border:1rpx solid #cacaca;text-align:center;}
.fabu-imgteam{font-size:28rpx; width:25%;margin:10rpx 10rpx; text-align:left;padding-left:30rpx}
...
.fabu-header-left{width:140rpx;height:140rpx;margin:20rpx;background:#FF495E;border-radius: 90rpx;overflow: hidden;}
...
.fabu-name{font-size:28rpx;width:100%; text-align:left;height:50%;flex-wrap:wrap-reverse;display:flex;
	}
.fabu-userinfo{
	width:100%;
	line-height:30rpx;
...
.fabu-more{font-size:20rpx;width:50%;margin:10rpx 2%;border-radius:20rpx;border:1rpx solid #ffaa00;text-align:center;}
...
.fabu-three{width:100%;min-height:1000rpx;background:#ffffff;padding-top:30rpx;padding-bottom:100rpx}
.footer{
	height: 100upx;
	position: fixed;
	bottom: 0;
