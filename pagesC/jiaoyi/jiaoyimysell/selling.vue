	...
	<view>
		<view class="mybuy border-b" v-for="(item,index) in list" :key="index">
			...
			<view class="list-ul bt30">
			   ...
			   <view class="list-li hhimg" v-else-if="item.status == 0">
			   ...
			   <view class="list-li hhimg">
				   ...
				   <view class="hhok" v-if="item.status == 0" @click="buyclick(item.id,5)">待付款</view>
				   <view class="hhok" v-if="item.status == 2" @click="buyclick(item.id,4)">确认收款</view>
				   <view class="hhok" v-if="item.status == 1">已完成</view>
			   </view>
			</view>
		</view>				
	</view>
