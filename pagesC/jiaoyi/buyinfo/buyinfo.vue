<template>
	<view>
		     <view class="content">
				<view class="list-ul pt20">
 				...
 				<view class="list-ul">
					 ...
					 <view class="list-right">{{snumber}} (请确保帐户{{allnumber}}点)</view>
				</view> 
				<view class="list-ul">
					 <view class="list-left">价格</view>
					 <view class="list-right">{{samount}}元</view>
				</view>
			</view>
			<view class="content">
				<view class="list-ul pt20">
					 <view class="list2-left">卖出数量</view>
					 <view class="list2-right">
						 <input type="number"  class="inp-100" name="number" v-model="number"  placeholder=" 请输入卖出数量" />
				     </view>
				</view>
				<view class="list-ul pt20">
				...
				<view class="list-ul pt20">
					 <view class="list2-left">收款姓名</view>
					 <view class="list2-right">
				...
				<view class="list-ul">
					 <view class="list2-left">帐号昵称</view>
					 <view class="list2-right">
						 {{username}}
				     </view>
				</view>
				<view class="list-ul">
					 <view class="list2-left">手机号码</view>
					 <view class="list2-right">
						{{mobile}}
					 </view>
				</view>					
			...
			<view class="mp690">
				<button  class="loading-btn"  @click="buyocc()">确认售出</button>
			</view>	
	</view>
</template>
...
<script>
	...
	export default {
		...
		methods: {
				...
				uni.request({
					...
					success: (res) => {
						if(res.data.status == 'success'){
							...
							this.snumber = res.data.data.trade.remaining_number
							this.number  =  res.data.data.trade.remaining_number
							this.min_number  =  res.data.data.trade.min_number
							this.samount = Math.round(res.data.data.trade.remaining_amount)
							...
							this.allnumber=(res.data.data.trade.remaining_number*1.1).toFixed(1)
						}else{
			...
			buyclick(){
				if(!this.number){
					uni.showToast({ title: '卖出数量不能为空', icon: "none" });
					return false;
				}
				if(!this.bank){
...
.mt20{margin-top:10rpx}
.pt20{padding-top:10rpx}
