...
export default {
	...
	methods: {
		...
		$sendWithPingToken(cb){
			this.$socketSend({'action':'ping','data':_data.localData('lytoken')},cb);
		},
		$reset(){
			params.status = false;
		...
		$requestSend(config){
			...
			send_data['_token'] = _data.localData('lytoken');
